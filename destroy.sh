...
desktop_image_name=$(az image list --resource-group packer-rg --query "[?starts_with(name, 'desktop_image')]|sort_by(@, &name)[-1].name" --output tsv)
...
cd 03-deploy
terraform init
terraform destroy -var="games_image_name=$games_image_name" -var="desktop_image_name=$desktop_image_name" -auto-approve
cd ..
